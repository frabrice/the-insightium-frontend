/*
  # Insightium Database Schema

  1. New Tables
    - `categories` - Content categories
    - `articles` - Main article content
    - `videos` - Video content for magazine and TV show
    - `podcast_episodes` - Podcast episode content
    - `main_articles` - Main featured articles
    - `featured_articles` - Featured articles section
    - `editors_pick` - Editor's pick articles
...
BEGIN
  ...
  IF main_article_id IS NOT NULL THEN
    ...
    ON CONFLICT (position) DO UPDATE SET article_id = main_article_id;
  END IF;
  
  IF second_article_id IS NOT NULL THEN
    INSERT INTO main_articles (article_id, position) 
    VALUES (second_article_id, 'second')
    ON CONFLICT (position) DO UPDATE SET article_id = second_article_id;
  END IF;
END $$;
