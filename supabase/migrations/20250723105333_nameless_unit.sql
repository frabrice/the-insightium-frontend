/*
  # TV Show and Podcast Management System

  1. Enhanced Tables
    - Update `videos` table for TV show episodes
    - Update `podcast_episodes` table with additional fields
    - Add management tables for better organization
  
  2. New Management Tables
    - `tv_show_seasons` - Organize episodes by seasons
...
-- Grant permissions
GRANT ALL ON tv_show_seasons TO authenticated;
GRANT ALL ON guest_profiles TO authenticated;
GRANT ALL ON podcast_series TO authenticated;

GRANT SELECT ON tv_show_seasons TO anon;
GRANT SELECT ON guest_profiles TO anon;
GRANT SELECT ON podcast_series TO anon;
...
-- Update existing videos with season information
DO $$
DECLARE
  season1_id uuid;
  season2_id uuid;
BEGIN
  -- Get season IDs
  SELECT id INTO season1_id FROM tv_show_seasons WHERE season_number = 1;
  SELECT id INTO season2_id FROM tv_show_seasons WHERE season_number = 2;
  
  -- Update existing videos with season and episode numbers
  UPDATE videos 
  SET 
    season_id = season1_id,
    episode_number = CASE 
      WHEN title LIKE '%Episode 12%' THEN 12
      WHEN title LIKE '%Episode 11%' THEN 11
      ELSE 1
    END,
    featured = CASE WHEN is_new = true THEN true ELSE false END
  WHERE section != 'Magazine' AND season_id IS NULL;
END $$;

-- Update existing podcast episodes with guest and series information
DO $$
DECLARE
  kwame_id uuid;
  amina_id uuid;
  marcus_id uuid;
  sarah_id uuid;
  leaders_series_id uuid;
BEGIN
  -- Get guest IDs
  SELECT id INTO kwame_id FROM guest_profiles WHERE full_name = 'Dr. Kwame Asante';
  SELECT id INTO amina_id FROM guest_profiles WHERE full_name = 'Prof. Amina Hassan';
  SELECT id INTO marcus_id FROM guest_profiles WHERE full_name = 'Marcus Johnson';
  SELECT id INTO sarah_id FROM guest_profiles WHERE full_name = 'Dr. Sarah Ochieng';
  
  -- Get series ID
  SELECT id INTO leaders_series_id FROM podcast_series WHERE title = 'Library Talk: Education Leaders';
  
  -- Update existing podcast episodes
  UPDATE podcast_episodes 
  SET 
    guest_id = CASE 
      WHEN guest = 'Dr. Kwame Asante' THEN kwame_id
      WHEN guest = 'Prof. Amina Hassan' THEN amina_id
      WHEN guest = 'Marcus Johnson' THEN marcus_id
      WHEN guest = 'Dr. Sarah Ochieng' THEN sarah_id
      ELSE NULL
    END,
    series_id = leaders_series_id,
    episode_number = CASE 
      WHEN title LIKE '%AI in Education%' THEN 1
      WHEN title LIKE '%Inclusive Learning%' THEN 2
      WHEN title LIKE '%Storytelling%' THEN 3
      WHEN title LIKE '%Mental Health%' THEN 4
      ELSE 1
    END,
    featured = CASE WHEN plays::int > 10000 THEN true ELSE false END,
    spotify_url = 'https://open.spotify.com/episode/example',
    apple_url = 'https://podcasts.apple.com/episode/example',
    google_url = 'https://podcasts.google.com/episode/example'
